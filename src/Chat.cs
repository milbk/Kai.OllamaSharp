...
namespace OllamaSharp
{
    public class Chat
    {
        ...
        /// <param name="cancellationToken">The token to cancel the operation with</param>
        public Task<IEnumerable<Message>> Send(string message, IEnumerable<string> imagesAsBase64,
        ...
        /// <param name="message">The message to send</param>
        /// <param name="cancellationToken">The token to cancel the operation with</param>
        public Task<IEnumerable<Message>> SendAs(ChatRole role, string message,
            CancellationToken cancellationToken = default) => SendAs(role, message, null, cancellationToken);

        public Task<IEnumerable<Message>> SendAs(ChatRole role, string message,
            CancellationToken cancellationToken = default, RequestOptions options = null)
        {
            if (options == null) return SendAs(role, message, null, cancellationToken);
            return SendAs(role, message, null, options, cancellationToken);
        }
